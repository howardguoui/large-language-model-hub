...
const BADGE: Record<string, { label: string; color: string }> = {
  ...
  'visual':       { label: 'Visual',   color: 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300' },
  ...
  return (
    <aside className="w-64 shrink-0 bg-white dark:bg-slate-900 border-r border-gray-200 dark:border-slate-800 flex flex-col overflow-hidden">
      ...
      <div className="p-3 border-b border-gray-200 dark:border-slate-800">
        <div className="flex items-center gap-2 bg-gray-100 dark:bg-slate-800 rounded-lg px-3 py-2 text-sm focus-within:ring-2 focus-within:ring-indigo-500">
            ...
            <div key={chapter.id} className="mb-1">
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 text-sm font-semibold transition-colors hover:bg-gray-100 dark:hover:bg-slate-800 ${
                  hasActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-700 dark:text-slate-300'
                }`}
              >
              ...
              {!isCollapsed && (
                <div className="pl-2">
                  {filtered.map(topic => {
                    ...
                    const badge = BADGE[topic.contentType]
                    return (
                      <Link
                        key={topic.id}
                        to={`/learn/${topic.id}`}
                      ...
                      >
                        ...
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium shrink-0 ${
                          isActive ? 'bg-indigo-500 text-indigo-100' : badge.color
                        }`}>
                          {badge.label}
                        </span>
                      </Link>
